...
contract("Token airdrop using balances contract", async function() {
	var sut;
	before(async function(){
		sut = await createSUT();
	})
	...
	it("Should mint new tokens using admin contract", async function() {
		var result = await sut.adminContract.adminCreateTokens(web3.toWei(10000000, 'ether'), testAccounts.admin, {from: testAccounts.deployer});
		assert(result);
	});
	...
	it("Should transfer tokens to the provided addresses", async function() {
		var totalSent = [];
		// for(var i = 0; i < 50; i++){
		// 	var temp;
		// 	recieverAddresses.push(web3.eth.accounts[i]);
		...
		// 	totalSent.push(temp);
		// 	tokenAmount.push(web3.toWei(temp, 'ether'));
		// 	// console.log(`Address : ${recieverAddresses[i]} ==> Amount(in wei): ${tokenAmount[i]} ==> Amount: ${temp}`)
		// }
		// tokenAmount.push(web3.toWei(39, 'ether'));
		tokenAmount.push(web3.toWei(20, 'ether'));
		// recieverAddresses.push(web3.eth.accounts[0]);
		recieverAddresses.push(web3.eth.accounts[2]);
		await sut.airDropContract.airdropTokenUsingBalances(tokenAmount, recieverAddresses, sut.balanceContract.address, testAccounts.admin);
		var totalTokenSent = totalSent.reduce((a, b) => a + b, 0);
		var totalSupply = await sut.humanStandardTokenContract.totalSupply();
		var total = await sut.humanStandardTokenContract.balanceOf(testAccounts.admin);
		// var recieverTotal = await sut.humanStandardTokenContract.balanceOf(recieverAddresses[1]);
		// console.log("address " + recieverAddresses[1] + " ======= " + recieverTotal + "------ " + tokenAmount[1] + " ------ " + totalSent[1])
		// console.log("toWei " + web3.toWei(totalSent[1], 'ether'))
		// console.log("fromWei " + web3.fromWei(totalSent[1], 'ether'))
		// console.log(web3.fromWei(total, 'ether'), web3.fromWei(totalSupply, 'ether'), totalSent[1])
		// console.log(web3.fromWei(total, 'ether') + " ======= " + web3.fromWei(totalSupply, 'ether') - totalTokenSent)
		// assert(total == web3.toWei(10000000));
	})

	it("Should return the balance of particular addresses", async function(){
		for(var i = 0; i < tokenAmount.length; i++){
			var balance = await sut.humanStandardTokenContract.balanceOf(recieverAddresses[i]);
			console.log(web3.fromWei(balance, 'ether') + " =======> " + web3.fromWei(tokenAmount[i]));
		}
	})
})
