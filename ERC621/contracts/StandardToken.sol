...
contract StandardToken is Token{
	...
	function increaseTotalSupply(address _to, uint _value) public view returns(bool) {
		balances.incTotalSupply(_value);
		balances.incBalance(_to, _value);
		return true;
	}

	function decreaseTotalSupply(address _from, uint _value) public view returns(bool) {
		if(balances.getBalance(_from) >= _value){
			balances.decTotalSupply(_value);
