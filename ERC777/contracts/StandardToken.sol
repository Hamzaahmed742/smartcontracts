...
contract StandardToken is ERC20Token, ERC777Token, ERC820Implementer {
    using SafeMath for uint256;

    string public mName;
    string public mSymbol;
    uint256 public mGranularity;
    address public owner;
    bool public mErc20compatible;
    ...
    function totalSupply() public constant returns (uint256) { return balances.getTotalSupply(); }

    /// @notice Return the account balance of some account
    /// @param _tokenHolder Address for which the balance is returned
    /// @return the balance of `_tokenAddress`.
    function balanceOf(address _tokenHolder) public constant returns (uint256) { return balances.getBalance(_tokenHolder); }
    ...
    function transferFrom(address _from, address _to, uint256 _amount) public erc20 returns (bool success) {
        if (balances.getBalance(msg.sender) >= _amount && balances.getAllowance(msg.sender, _to) >= _amount && _amount > 0) {
            balances.decApprove(msg.sender, _to, _amount);
    ...
    function approve(address _spender, uint256 _amount) public erc20 returns (bool success) {
        balances.setApprove(msg.sender, _spender, _amount);
        ...
        private
    {
        ...
        require(balances.getBalance(_from) >= _amount); // ensure enough funds

        balances.decBalance(_from, _amount);
        balances.incBalance(_to, _amount);
